...
const LandingNavbar = () => {
  ...
  return (
    <nav className="w-full bg-secondary flex py-6 justify-between items-center navbar">
      <a href="/">
      <h1 className="my-auto font-poppins font-bold text-[24px] text-primary rounded-lg">
        DavidUrschel.com
      </h1>
      </a>
      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`font-poppins font-normal cursor-pointer text-[16px] no-underline hover:underline text-primary ${
